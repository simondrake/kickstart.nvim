...
(import_statement
  ...
  vim.api.nvim_buf_set_text(bufnr, start_line, 0, start_line, 0, { string.format("import %s from '%s'", imported_items, package), '', '' })
  return true
end
  ...
  for _, match, _ in query:iter_matches(tree:root(), bufnr) do
    for id, node in pairs(match) do
      ...
      if name == 'import_clause' then
        ...
        vim.api.nvim_buf_set_text(bufnr, start_row, start_col + 1, start_row, start_col + 1, { string.format('%s, ', variable_name) })
      end
    end
  end
